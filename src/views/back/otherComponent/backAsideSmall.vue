...
export default {
  ...
  setup () {
    ...
    function addTab (asideItem) {
      ...
      for (let i = 0; i < frontObject.editableTabs.length; i++) {
        if (asideItem.id === frontObject.editableTabs[i].id) {
          frontObject.editableTabsValue = asideItem.name
          return
        }
      }
      // 增加标签页
      frontObject.editableTabs.push({
        id: asideItem.id,
